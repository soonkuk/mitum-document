...
type CreateAccountsItemProcessor struct {
	...
	ns   state.State                // target acccount new account state
	nb   map[CurrencyID]AmountState // target account new balance state
}
...
type CreateAccountsProcessor struct {
	cp *CurrencyPool
	CreateAccounts
	sb       map[CurrencyID]AmountState // sender balance state
