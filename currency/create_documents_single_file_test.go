...
func (t *testCreateDocumentsSingleFile) TestNew() {
	...
	docKey, err := NewKey(docPrvk.Publickey(), 100)
	t.NoError(err)
	// owner key
	ownerKey, err := NewKey(ownerPrvk.Publickey(), 100)
	t.NoError(err)

	// document keys(keys, threshold)
	docKeys, _ := NewKeys([]Key{docKey}, 100)
	// owner keys
	ownerKeys, _ := NewKeys([]Key{ownerKey}, 100)
	// owner address calculated from owner keys
	owner, _ := NewAddressFromKeys(ownerKeys)
...
func testCreateDocumentsSingleFileEncode(enc encoder.Encoder) suite.TestingSuite {
	...
	t.newObject = func() interface{} {
		...
		ownerKey, err := NewKey(ownerPrvk.Publickey(), 100)
		t.NoError(err)
		docKeys, err := NewKeys([]Key{docKey}, 100)
		t.NoError(err)
		ownerKeys, err := NewKeys([]Key{ownerKey}, 100)
		t.NoError(err)
