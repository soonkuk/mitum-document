...
func (t *testFileData) TestNew() {
	sc := SignCode("ABCD")
	sKey, _ := NewKey(key.MustNewBTCPrivatekey().Publickey(), 100)
	sKeys, _ := NewKeys([]Key{sKey}, 100)
	sOwner, _ := NewAddressFromKeys(sKeys)

	a := MustNewFileData(sc, sOwner)
	t.Equal(a, a.WithData(sc, sOwner))

	nKey, _ := NewKey(key.MustNewBTCPrivatekey().Publickey(), 100)
	nKeys, _ := NewKeys([]Key{nKey}, 100)
	nOwner, _ := NewAddressFromKeys(nKeys)
