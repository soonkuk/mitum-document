...
func (opr *OperationProcessor) Process(op state.Processor) error {
	...
	case *currency.TransfersProcessor,
		...
		*blocksign.CreateDocumentsProcessor,
		*blocksign.TransferDocumentsProcessor:
		return opr.process(op)
	case currency.Transfers, currency.CreateAccounts, currency.KeyUpdater, currency.CurrencyRegister, currency.CurrencyPolicyUpdater, blocksign.CreateDocuments, blocksign.TransferDocuments:
...
func (opr *OperationProcessor) process(op state.Processor) error {
	...
	case *currency.KeyUpdaterProcessor:
		sp = t
	case *blocksign.CreateDocumentsProcessor:
		sp = t
	case *blocksign.TransferDocumentsProcessor:
		sp = t
	default:
		return op.Process(opr.pool.Get, opr.pool.Set)
	}
